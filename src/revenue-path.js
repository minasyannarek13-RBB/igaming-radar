...
export function classifyDomainLanding(input) {
  ...
  const result = {
    state: 'HEALTHY', scope: 'none', attributable: false, cause: 'NOT_OBSERVABLE', dependencyEdges: 0,
  ...
  if (observations.probeContext === 'automated' && [403, 451].includes(observations.http)) {
    ...
    if (!corroborated) return { ...result, state: 'NOT_OBSERVABLE', scope: observations.page === 'challenge' ? 'probe-ambiguous' : 'geo-ambiguous' };
  }
  ...
  if (observations.dns === 'fail') {
    ...
    if (observations.probeContext === 'automated' || independentHealthyControls.length > 0) return { ...result, state: 'NOT_OBSERVABLE', scope: 'dns-probe-ambiguous' };
  ...
  if (observations.tls === 'fail') {
    ...
    if (corroborated) return { ...result, state: 'BROKEN', scope: 'target-corroborated' };
    const independentHealthyControls = controls.filter((control) => healthyControl(control, geo));
    if (observations.probeContext === 'automated' || independentHealthyControls.length > 0) return { ...result, state: 'NOT_OBSERVABLE', scope: 'tls-probe-ambiguous' };
  ...
  if (observations.http === 200 && observations.page === 'error-template') {
    ...
    const independentHealthyControls = controls.filter((control) => healthyControl(control, geo));
    if (observations.probeContext === 'automated' || independentHealthyControls.length > 0) return { ...result, state: 'NOT_OBSERVABLE', scope: 'soft-200-probe-ambiguous' };
    return { ...result, state: 'BROKEN', scope: 'landing' };
  }

  if ([403, 451].includes(observations.http) && observations.page === 'unavailable') {
    const mirrorHealthy = controls.some((control) => control?.target && control.geo === geo && control.state === 'HEALTHY');
    ...
    if (independentGeoControls.length >= 2) return { ...result, state: 'BROKEN', scope: 'geo-local-observed' };
...
export function initialRevenuePathLifecycle() {
  return { state: 'HEALTHY', incidentOpen: false, firstDetected: null, lastObservedAt: null, recoveryCandidateAt: null, healthyConfirmations: 0, recoveredAt: null, incidentOpenDurationMs: 0, observedExposureUpperBoundMs: null };
}
...
export function advanceRevenuePathLifecycle(previous, classified, observedAt, options = {}) {
  ...
  if (unhealthy) {
    const firstDetected = current.incidentOpen && current.firstDetected ? current.firstDetected : new Date(timestamp).toISOString();
    const firstDetectedMs = new Date(firstDetected).getTime();
    return { ...next, state: classified.state, incidentOpen: true, firstDetected, recoveryCandidateAt: null, healthyConfirmations: 0, recoveredAt: null, incidentOpenDurationMs: Math.max(0, timestamp - firstDetectedMs), observedExposureUpperBoundMs: null };
  }

  if (!current.incidentOpen) return { ...next, state: classified.state, healthyConfirmations: classified.state === 'HEALTHY' ? current.healthyConfirmations + 1 : 0 };

  if (classified.state === 'NOT_OBSERVABLE') return { ...next, state: current.state, healthyConfirmations: 0, recoveryCandidateAt: null, observedExposureUpperBoundMs: null };
  ...
  if (confirmations < recoveryConfirmations) return { ...next, state: current.state, healthyConfirmations: confirmations, recoveryCandidateAt, incidentOpenDurationMs: Math.max(0, timestamp - firstDetectedMs), observedExposureUpperBoundMs };

  return { ...next, state: 'HEALTHY', incidentOpen: false, firstDetected: current.firstDetected, healthyConfirmations: confirmations, recoveryCandidateAt, recoveredAt: new Date(timestamp).toISOString(), incidentOpenDurationMs: Math.max(0, timestamp - firstDetectedMs), observedExposureUpperBoundMs };
}
