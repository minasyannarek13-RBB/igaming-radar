...
async function probeCriticalAssets(assetUrls, transport) {
  ...
  for (const assetUrl of assetUrls.slice(0, MAX_ASSETS)) {
    try {
      const target = normalizeTarget(assetUrl);
      const { response, finalUrl } = await fetchPublicTarget(target, transport);
      ...
      evidence.push({
        ...
        provenance: 'Not observable externally',
        detail: String(error?.message || 'asset_probe_failed').slice(0, 160)
      });
    }
  }
...
export async function probeDomainLanding(input, { fetchImpl, lookupImpl, now = () => new Date() } = {}) {
  ...
  try {
    ...
    return {
      ...
      transportError: String(error?.message || 'probe_failed').slice(0, 160),
  ...
  const observations = {
    probeContext: 'automated',
    dns: 'ok',
    tls: finalUrl.protocol === 'https:' ? 'ok' : 'not_applicable',
    http: response.status,
    redirect: finalUrl.href === target.href ? 'none' : 'followed'
  };

  if ([403, 451].includes(response.status)) {
    observations.page = challengeMarkers.length > 0 && hasAnyMarker(body, challengeMarkers)
      ? 'challenge'
      : 'unavailable';
  } else if (response.status === 200 && errorMarkers.length > 0 && hasAnyMarker(body, errorMarkers)) {
    observations.page = 'error-template';
  } else {
  ...
  if (ctaMarkers.length > 0) {
    observations.cta = hasAnyMarker(body, ctaMarkers) ? 'present' : 'missing';
  }
  ...
  return {
    ...
    scope: classified.scope,
    cause: classified.cause,
    attribution: classified.attributable ? 'Inferred' : 'Not observable externally',
    dependencyEdges: classified.dependencyEdges,
    evidence: classified.evidence,
    roiProof: { status: 'NOT_CLAIMED', savedGgr: null, savedRevenue: null }
  };
}
