...
export async function runDomainLandingCycle(input, {
  ...
  for (let attempt = 0; attempt < maxCasAttempts; attempt += 1) {
    ...
    const stored = await store.compareAndSet(input.scopeId, probe.target, geo, currentRecord?.version ?? 0, lifecycle);
    if (!stored) continue;
    ...
    return { contract: 'domain-landing-cycle/v1', target: probe.target, geo, probe, lifecycle, alert, persistence: store.persistence ?? 'DURABLE_STORE', roiProof: { status: 'NOT_CLAIMED', savedGgr: null, savedRevenue: null } };
  }
