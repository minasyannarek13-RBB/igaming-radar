...
| HIGH-confidence false attribution | 0 | BLOCKED: HIGH independence currently deduplicates only by `sourceId`; two URLs from one provider/runtime provenance channel can satisfy HIGH |
...
- 2026-08-30: Evidence contract enforcement requires provenance, prevents synthetic/replay evidence from being labeled live. HIGH confidence requires genuinely independent Observed provenance; distinct `sourceId` values alone are not sufficient and remain a blocker until hardened.
