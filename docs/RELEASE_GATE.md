...
| Fabricated dependencies | 0 | BLOCKED: Entain runtime-looking `src` can create an edge without confirming the referenced JS resource loaded successfully |
...
## Evidence log
- 2026-08-30: Release Owner reconciled the latest Red Team finding against canonical `main` (`3fe8160d042b106d419ce70e68056551ad6eba20`), CI run `33300876492` (SUCCESS), and Vercel production deployment `dpl_3icPDFbSw3z4HDp3eqdtTtdG9UhP` (READY on the same SHA). The new Entain Sportsbook/Platform fingerprint currently treats an exact runtime-looking `src` on `scmedia.itsfogo.com` / `scmedia-us.itsfogo.com` as sufficient dependency evidence without confirming that the referenced JavaScript resource was successfully retrievable/loaded. This leaves a fabricated-edge path for a syntactically valid but nonexistent/blocked/timeout resource. Free Scan therefore remains CLOSED. Build Lane failing case: exact-pattern Entain `src` whose resource probe returns 404/403/timeout/DNS failure must produce zero Entain dependency edges; the hostname may remain Observed / UNATTRIBUTED. Attribution thresholds must not be weakened to resolve this blocker.
